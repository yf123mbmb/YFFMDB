...
//  GitHub: https://github.com/gaojunquan/YFFMDB
//
...
static YFFMDB *jqdb = nil;
+ (instancetype)shareDatabase
{
    return [YFFMDB shareDatabase:nil];
}

+ (instancetype)shareDatabase:(NSString *)dbName
{
    return [YFFMDB shareDatabase:dbName path:nil];
}
...
- (BOOL)jq_createTable:(NSString *)tableName dicOrModel:(id)parameters excludeName:(NSArray *)nameArr
{
    ...
    for (NSString *key in dic) {
        ...
        keyCount++;
        NSString* hao = [self GetLastTitle:fieldStr];

        if ((nameArr && [nameArr containsObject:key]) || [key isEqualToString:@"pkid"]) {
            
            if (keyCount == dic.count) {
                NSString *lastStr = [fieldStr substringFromIndex:fieldStr.length-1];//
                if([lastStr isEqualToString:@","]){
                    fieldStr = [[NSMutableString alloc] initWithFormat:@"%@", [fieldStr substringToIndex:fieldStr.length-1]];
                }
        ...
        if (keyCount == dic.count) {
            [fieldStr appendFormat:@" %@%@ %@ )", hao,key, dic[key]];
        }else{
            [fieldStr appendFormat:@" %@%@ %@", hao,key, dic[key]];
        }
...
-(NSString *)GetLastTitle:(NSString*)fieldStr{
    NSString *lastStr = [fieldStr substringFromIndex:fieldStr.length-1];//
    NSString *hao = [lastStr isEqualToString:@","] ? @"" : @",";
    return  hao;
}
...
//    va_end(args);
    ...
    NSDictionary *dic;
    NSMutableString *finalStr = [[NSMutableString alloc] initWithFormat:@"select * from %@ %@", tableName, format];
    NSArray *clomnArr = [self getColumnArr:tableName db:_db];
